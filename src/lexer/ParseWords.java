	...
	private boolean matchString() throws IOException {
		...
		while(true) {
			...
			if(key == '"') {
				...
				ErrorWriter.errorList.add(new ErrorType("字符串跨行", line, matched.substring(0, matched.length()-1)));
				nextLine();
				line++;
				return true;
			}else
		...
		SymbolTable.wordItemList.add(new WordItem(matched,search("STRING_VALUE") , line));
		return true;
	}
			...
			while(true){
				if (key == '\uFFFF')//判断是否读取文档结束
					...
					if (encode != -1){
						...
						SymbolTable.wordItemList.add(new WordItem(buffer.toString(), search("ID"), line));
					}
					buffer.setLength(0);
				}
					...
					if (x == 0 ) {//整数
						...
						if (number >= Integer.MIN_VALUE && number <= Integer.MAX_VALUE){
							SymbolTable.wordItemList.add(new WordItem(num, search("INTEGER_VALUE"), line));
						}else {
							ErrorWriter.addError("数字太大越界", line, ""+buffer.toString());
						}
						buffer.setLength(0);
					}else if(x == 1){//浮点数
						SymbolTable.wordItemList.add(new WordItem(buffer.toString(), search("FLOAT_VALUE"), line));
						buffer.setLength(0);
					}else {
