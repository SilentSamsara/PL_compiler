...
final public class Grammar {
	public static List<Production> productions = new ArrayList<Production>(); // 产生式列表，保存了多条产生式后就叫做文法。
	public static List<First> firsts = new ArrayList<First>(); // 文法的First集
	public static List<Follow> follows = new ArrayList<Follow>(); // 文法的Follow集
	public static List<String> terminals = new ArrayList<String>();
	public static List<String> non_terminals = new ArrayList<String>();
	...
	private void createterminals() {
		...
		while (index < productions.size()) {
			...
			while (temp < productions.get(index).rightParts.size()) {
				...
				String xString = ""+indexString;
				if(xString.equals(":")&&productions.get(index).rightParts.get(temp+1).equals("=")) {
					productions.get(index).rightParts.set(temp, ":=");
					temp++;
